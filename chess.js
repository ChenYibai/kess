...
[7, 105, 4], // 12 Cobblestone
[5, 27, 2], // 13 Iron Block
];
...
var blockSprite = new Map([[12, [10, 60]], [13, [18, 58]]]);
...
function assess() {
  ...
  for (var i = 0; i < n; i++) for (var j = 0; j < n; j++) {
    ...
    addition += Math.max(0, wt * 0.18 * (board[i][j].attack - 4)); // Swords
    addition += wt * 0.23 * (board[i][j].armor); // Armor
    ...
    res[t] += (addition * (board[i][j].blood / 10) + wt * (kingAlive[t] ? 1 : board[i][j].blood / 10));
  }
